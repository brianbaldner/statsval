...
namespace ValAPINet
{
    public class UserPresence
    {
        ...
        public static Presence GetUserPresence(string id)
        {
            string lockfile = "";
            ...
            UserPresence au = JsonConvert.DeserializeObject<UserPresence>(getResp.Content);
            Presence ret = null;
            foreach (Presence pres in au.presences)
            {
                if (pres.puuid == id)
                {
                    ret = pres;
            ...
            ret.privinfo = JsonConvert.DeserializeObject<priv>(Encoding.UTF8.GetString(Convert.FromBase64String(ret.@private)));
            return ret;
        }
    }
}
